...
export const LESSONS: readonly Lesson[] = [
  ...
  },
  {
    ...
    tryThis: ":noul is_urgent The message conveys urgency or time-sensitivity",
  },
  {
  ...
  },
  {
    ...
    tryThis:
      ":noul is_urgent The message conveys urgency | yes: Explicit deadline, or money being lost right now | no: Routine question with no time pressure",
  },
  {
    ...
    body: [
      ...
      "`:last` prints the last response body verbatim, next to the typed answers the client decoded from it.",
    ],
    tryThis: ":json",
  },
  {
  ...
  },
  {
    ...
    body: [
      ...
      "`:models` lists what the account can use, `:model jev-2` switches this session, and `:timeout 3` changes the per-attempt timeout the way `timeoutMs` does on a call.",
  ...
  },
  {
    ...
    body: [
      "`:ts` prints this session as a TypeScript program against the client this package ships — the same state, questions and model, with the answer lookups filled in.",
      "(`:rust` prints the same session against the Rust SDK, if that is where it is going.)",
